import { COLORMAP_LUTS } from "./colormap-data";

export const COLORMAP_NAMES = Object.keys(COLORMAP_LUTS);

/**
 * Build a 256-entry RGB LUT (Uint8Array, length 768) from an exact flat RGB
 * table generated by scripts/gen_colormaps.py. The generated tables already
 * have 256 entries sampled from matplotlib, so no interpolation is needed.
 */
function createColormapLUT(flatRGB: number[]): Uint8Array {
  const lut = new Uint8Array(256 * 3);
  // Generated tables are exactly 768 long; copy directly (clamps to 256 entries).
  const n = Math.min(flatRGB.length, lut.length);
  for (let i = 0; i < n; i++) lut[i] = flatRGB[i];
  return lut;
}

export const COLORMAPS: Record<string, Uint8Array> = Object.fromEntries(
  Object.entries(COLORMAP_LUTS).map(([name, flatRGB]) => [name, createColormapLUT(flatRGB)])
);
