...
def test_strain_map_2d_pure_rotation():
    ...
    # Sign convention: applying R(+θ) to the local lattice yields widget
    # ``theta = +sin θ ≈ +0.05`` (the rotation needed to map the LOCAL frame
    # back to the REFERENCE frame, equivalent to the rotation OF the reference
    # if we ran the convention the other way). This is the same sign as
    # py4DSTEM's ``get_strain_from_reference_g1g2`` decomposition; lock it
    # so a future refactor cannot silently flip it.
